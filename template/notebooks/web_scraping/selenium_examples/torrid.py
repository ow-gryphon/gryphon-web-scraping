...
class Torrid(BaseScraper):
    @property
    def output_directory(self):
        return Path('data/output/torrid')
    ...
    def navigate_to_required_clothing_sections(self):
        ...
        links = self._browser.find_elements_by_xpath("//li[@class='nav-item']/a[contains(@href,'/clothing/')]")
        ...
        for link_url, link_text in zip(link_urls, link_texts):
            ...
            if filename.exists():
                ...
                while more_pages:
                    ...
                    try:
                        next_link = self._browser.find_element_by_xpath("//a[@class='page-next']")
                        next_href = next_link.get_attribute("href")
                        if next_href == "" or next_href is None:
                            more_pages = False
                        else:
                            counter += 1
                            self._browser.get(next_href)
                    except Exception:
                        more_pages = False
                self.save_data(items, filename)

    def get_current_page_items(self, category):
        outermost_xpath = "//div[@class='product-tile product-image-container']"
        product_id_string = "data-itemid"
        product_name_xpath = "./div[@class='product-name']/h2/a"
        product_price_xpath = "./div[@class='product-pricing']/div[@class='product-price']/div[@class='product-discounted-price']"
        product_sale_price_xpath = "/span[@title='Sale Price']"
        product_regular_price_xpath = "/span[@title='Regular Price']"
        ...
        for element in product_web_elements:
            ...
            try:
                ...
                product_name = self.clean_string(element.find_element_by_xpath(product_name_xpath).text)
                product_sale_price = self.clean_string(element.find_element_by_xpath(product_price_xpath +
                                                                                     product_sale_price_xpath).text)
                product_reg_price = self.clean_string(element.find_element_by_xpath(product_price_xpath +
                                                                                    product_regular_price_xpath).text)
